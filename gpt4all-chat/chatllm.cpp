...
bool ChatLLM::loadModel(const ModelInfo &modelInfo)
{
    ...
    if (fileInfo.exists()) {
        if (isChatGPT) {
            ...
            if (m_llModelInfo.model) {
                ...
                if (!success) {
                    ...
                    m_llModelInfo = LLModelInfo();
                    emit modelLoadingError(QString("Could not load model due to invalid model file for %1").arg(modelInfo.name));
                } else {
                    ...
                    default:
                        {
                            ...
                            if (!m_isServer)
                                LLModelStore::globalInstance()->releaseModel(m_llModelInfo); // release back into the store
                            m_llModelInfo = LLModelInfo();
                            emit modelLoadingError(QString("Could not determine model type for %1").arg(modelInfo.name));
                        }
                    }
                }
            } else {
                if (!m_isServer)
                    LLModelStore::globalInstance()->releaseModel(m_llModelInfo); // release back into the store
                m_llModelInfo = LLModelInfo();
                emit modelLoadingError(QString("Could not load model due to invalid format for %1").arg(modelInfo.name));
            }
        }
#if defined(DEBUG_MODEL_LOADING)
        ...
        if (!m_isServer)
            LLModelStore::globalInstance()->releaseModel(m_llModelInfo); // release back into the store
        m_llModelInfo = LLModelInfo();
        emit modelLoadingError(QString("Could not find file for model %1").arg(modelInfo.name));
    }
